...
5. You will need to enable egress to two control plane URLs. The listener polls these endpoints for deployments (use the pair that matches your LangSmith region—EU (GCP): `eu.api.host` / `eu.api.smith`; US (AWS): `aws.api.host` / `aws.api.smith`; APAC (GCP): `apac.api.host` / `apac.api.smith`):
...
4. Configure your `langgraph-dataplane-values.yaml` file.
    ```bash
      config:
        ...
        hostBackendUrl: "https://api.host.langchain.com" # Override for EU (GCP) (`eu.api.host...`), US (AWS) (`aws.api.host...`), or APAC (GCP) (`apac.api.host...`)
        smithBackendUrl: "https://api.smith.langchain.com" # Override for EU (GCP) (`eu.api.smith...`), US (AWS) (`aws.api.smith...`), or APAC (GCP) (`apac.api.smith...`)
