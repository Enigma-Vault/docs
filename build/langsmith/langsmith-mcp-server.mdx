...
LangSmith Cloud now hosts an OAuth-authenticated remote MCP server at `https://api.smith.langchain.com/mcp`, with regional endpoints for EU (`https://eu.api.smith.langchain.com/mcp`), AWS-hosted US (`https://aws.api.smith.langchain.com/mcp`), and APAC (`https://apac.api.smith.langchain.com/mcp`). It exposes the same tool surface as the standalone server documented on this page, but authenticates via OAuth 2.1 with dynamic client registration—no API key, no separate deployment, no header configuration.
...
Use the **hosted** MCP server to connect to [LangSmith Cloud](/langsmith/cloud) (`smith.langchain.com`, `eu.smith.langchain.com`, `aws.smith.langchain.com`, or `apac.smith.langchain.com`). To connect to Cloud or [self-hosted LangSmith](/langsmith/self-hosted), run the server [locally](#installation-run-locally) and set `LANGSMITH_ENDPOINT`. For self-hosted deployments, you can also run the server via the [Docker image](#docker-deployment-http-streamable) inside your VPC.
