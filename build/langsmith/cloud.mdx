...
<Note>
This section is only relevant for cloud-managed LangSmith at [https://smith.langchain.com?utm_source=docs&utm_medium=cta&utm_campaign=langsmith-signup&utm_content=langsmith-cloud](https://smith.langchain.com), [https://eu.smith.langchain.com](https://eu.smith.langchain.com), [https://aws.smith.langchain.com](https://aws.smith.langchain.com), and [https://apac.smith.langchain.com](https://apac.smith.langchain.com).
...
<Note>
LangSmith SaaS is available in APAC on GCP in `australia-southeast1` (Sydney).
</Note>

### Regional storage

The resources and services in this table are stored in the location corresponding to the URL where sign-up occurred (US (GCP), EU (GCP), US (AWS), or APAC (GCP)). Cloud-managed LangSmith uses [Supabase](https://supabase.com) for authentication/authorization and [ClickHouse Cloud](https://clickhouse.com/cloud) for the data warehouse.

|                                     | US (GCP)                                                           | EU (GCP)                                                                 | US (AWS)                                                                  | APAC (GCP)                                                                       |
| ----------------------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------------ | ------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| URL                                 | [https://smith.langchain.com?utm_source=docs&utm_medium=cta&utm_campaign=langsmith-signup&utm_content=langsmith-cloud](https://smith.langchain.com)         | [https://eu.smith.langchain.com](https://eu.smith.langchain.com)         | [https://aws.smith.langchain.com](https://aws.smith.langchain.com)        | [https://apac.smith.langchain.com](https://apac.smith.langchain.com)             |
| API URL                             | [https://api.smith.langchain.com](https://api.smith.langchain.com) | [https://eu.api.smith.langchain.com](https://eu.api.smith.langchain.com) | [https://aws.api.smith.langchain.com](https://aws.api.smith.langchain.com) | [https://apac.api.smith.langchain.com](https://apac.api.smith.langchain.com)     |
| Cloud                               | GCP us-central1 (Iowa)                                             | GCP europe-west4 (Netherlands)                                           | AWS us-east-2 (Ohio)                                                      | GCP australia-southeast1 (Sydney)                                                |
| Supabase                            | AWS us-east-1 (N. Virginia)                                        | AWS eu-central-1 (Germany)                                               | AWS us-east-2 (Ohio)                                                      | AWS ap-southeast-2 (Sydney)                                                      |
| ClickHouse Cloud                    | us-central1 (Iowa)                                                 | europe-west4 (Netherlands)                                               | us-east-2 (Ohio)                                                          | australia-southeast1 (Sydney)                                                    |
| [LangSmith deployment](/langsmith/deployment) | GCP us-central1 (Iowa); `*.us.langgraph.app`                       | GCP europe-west4 (Netherlands); `*.eu.langgraph.app`                     | AWS us-east-2 (Ohio); `*.aws.us.langgraph.app`                            | GCP australia-southeast1 (Sydney); `*.apac.langgraph.app`                        |
...
| US (GCP)       | EU (GCP)       | US (AWS)       | APAC (GCP)     |
| -------------- | -------------- | -------------- | -------------- |
| 34.59.65.97    | 34.13.192.67   | 18.188.147.158 | 34.151.89.217  |
| 34.67.51.221   | 34.147.105.64  | 18.219.86.202  | 34.116.97.4    |
| 34.46.212.37   | 34.90.22.166   | 3.21.57.192    | 34.151.162.199 |
...
The LangChain endpoints map to the following static IP addresses for traffic that terminates on our **GCP load balancers** (US/EU/APAC) or, for **US (AWS)**, on the **Network Load Balancer** in `us-east-2` (API and gateway hostnames):

| US (GCP)       | EU (GCP)       | US (AWS)      | APAC (GCP)     |
| -------------- | -------------- | ------------- | -------------- |
| 34.8.121.39    | 34.95.92.214   | 3.129.27.169  | 34.149.149.213 |
...
You may need to allowlist these to enable traffic from your private network to LangSmith SaaS endpoints (`api.smith.langchain.com`, `smith.langchain.com`, `beacon.langchain.com`, `eu.api.smith.langchain.com`, `eu.smith.langchain.com`, `eu.beacon.langchain.com`, `aws.api.smith.langchain.com`, `aws.smith.langchain.com`, `apac.api.smith.langchain.com`, `apac.smith.langchain.com`, `apac.beacon.langchain.com`).
