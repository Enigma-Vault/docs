...
- Fixed the 'Enabled' column width in the automations table to prevent header truncation.
- Updated the details view header in the frontend for improved user experience.
- Removed dead `run_stats_facets` join from session stats queries for better performance.
- Improved the responsiveness of the MCP server filter dropdown to make it scrollable.
- Made the Polly button icon responsive to cursor interaction near bounding boxes.
- Added created and updated sort options to evaluators in the backend.
- Improved the performance of loading the next page when already scrolled to the bottom of the DataGrid.
- Adjusted the responsive layout for the pretty feedback editor for better usability.
- Added URL allowlist enforcement for JWT injection in Polly authentication.
- Added support for Salesforce tools and fleet-admin read permissions behind feature flags.
- Updated evaluator templates and added user attribution for evaluator creation.
- Enhanced the sorting and filtering capabilities within the evaluators table.
- Added async agents and spend limit enforcement to the fleet, along with a weekly spend query feature.
- Fixed trace and feedback layout bugs in multiple UI instances including the full page trace view.
- Fixed an issue where evaluators did not respect resource application tags.
- Improved the UX for agent name and color selection in fleet settings.
- Implemented new API endpoints for bulk evaluator operations and the addition of run rules in the resources tab.
- Enhanced the new org members page with a feature flag and improved user cost table with agent/user toggle.
- Integrations into the agent builder with native support for Google Calendar and other Google tools.
- Added a Go endpoint for CRUD operations and spend limits for fleets.
- Added tracing projects and agent builder usage aggregation across fleet projects for better reporting.
- Introduced improved error logging and tracing span recording for robust debugging and monitoring.
- Added PWA support for mobile-friendly login and installable application experience.
- Enabled audit logging for all remaining Go and Python backend endpoints.
- Enhanced evaluator reuse UI and implemented several issue board improvements for better feedback handling.
- Added GitHub OAuth user authentication flow and installation synchronization.
- Introduced inclusive metrics and cost details filtering within SmithDB endpoints for advanced usage analysis.
- Implemented trace latency and cost detail display in message views for enhanced observability.
- Improved the performance of idle sessions sync and experiments sync processes.
- Added support for ClickHouse migrations with mutual TLS (mTLS).
- Added self-hosted license updates via new API endpoints for streamlined customer management.
...
- Fixed UI issue by widening the 'Enabled' column in the automations table to prevent header truncation.
- Updated UI to include solid icons and added an xs variant option.
- Improved details view header in the frontend.
- Enhanced performance by removing a dead join from session stats queries.
- Made the MCP server filter dropdown scrollable in the fleet UI.
- Updated evaluators with new sorting options for created and updated timestamps.
- Expanded support for URL restrictions by adding URL allowlist enforcement for JWT injection.
- Introduced a click-to-filter mechanism in the evaluators table for type, feedback key, and resource cells.
- Fixed various bugs including cursor issues in the polly section, responsive layout for feedback editors, and scroll issues in the evaluator table.
- Added support for evaluator actions in the feedback panel with dependency injection.
- Improved trace layout in the full-page trace view.
- Integrated new async agents feature, enhancing the conversation capabilities within fleets.
- Enhanced evaluator features by adding feedback key and resource filters, improving manageability and performance in the evaluators UI.
- Implemented new evaluator attribution features including created_by user attribution.
- Updated UI components for experiment comparison views, introducing additional filters and improved tracing visualization.
- Fixed issues with file creation interrupt states, preventing UI inconsistencies.
- Introduced a redesigned memory approval UX in fleets, refining user management capabilities.
- Provided a more robust evaluator reuse experience with various fixes and optimizations to UI elements.
- Enhanced API capabilities with the addition of Go CRUD endpoints for spend limits to improve resource management.
- Integrated new Google Calendar tools with optional calendar_id, improving the flexibility of scheduling tools.
- Integrated support for GitHub OAuth, aligning with security best practices for self-hosted environments.
- Added new API endpoints for managing and controlling audit logs, enhancing security and accountability.
- Optimized SQL queries by adding relevant indices, improving database performance.
- Migrated certain fleet capabilities to a new headless fleet architecture, improving modularity and scalability.
- Improved loading speeds and reduced latency across the UI with caching optimizations.
- Integrated new evaluator features with carousel follow-up views.
- Added support for data streaming CSV export in experiments, enriching data reporting capabilities.
- Streamlined organization member pages with new skeleton features and control updates.
...
- Fixed the 'Enabled' column header truncation issue in the automations table.
- Fixed a bug in sandboxing that impacted Docker builds with shared API types.
- Improved frontend icon package to include solid icons and added an icon XS variant.
- Updated the frontend details view header for better user experience.
- Improved performance by removing unnecessary join in session stats queries.
- Made the MCP server filter dropdown scrollable in the fleet UI.
- Enhanced UI to show a back button on the full page trace view.
- Fixed layout issues in full-page tracing views.
- Enabled sorting in the evaluators table by 'created' and 'updated' dates.
- Added URL allowlist enforcement for JWT injections in Polly.
- Included a backfill_id in run rule responses even when progress is not requested.
- Enhanced rbac with the `projects:update-retention` permission.
- Added the ability to detach run rules from the resources tab.
- Made Auth Endpoint selection responsive in non-SSO login scenarios.
- Added support for new OAuth tools and mocked fourth-party integrations.
- Removed provider gate blocking tokens fetch on self-hosted instances for better fleet integration.
- Enabled scroll for human messages in chat upon sending in fleet UI.
- Improved the sorting functionality and support intent schema on the frontend.
- Reworked frontend UX for evaluator management and increased its responsiveness.
- Updated tracing projects to default to thread group by for improved data organization.
- Improved the evaluator UX for fork interactions.
- Added the 'Flag Issue' button for issue agent reviews in the issues board.
- Enabled latency measurements on tool calls in trace messages.
- Introduced a new endpoint to update online self-hosted licenses.
- Added a limit for setting up Google calendar integration in the agent builder.
- Added features for snapshot capturing and viewport management in sandbox environments.
- Included detailed logs for memory profiling to aid in debugging.
- Added skill mention support for cron schedule triggers.
- Removed dead code and streamlined frontend components for better performance.
- Enhanced mobile login and PWA installability.
- Rendered cost information alongside token count, and improved displays of concurrent tool calls.
- Introduced new Postgres TLS with system root cert support.
- Integrated various UI improvements, specifically tool call renderers and button stylings.
- Implemented a dark mode gradient fix and made diverse UX improvements.
- Made backend and frontend evaluator management more intuitive and navigable.
