...
This category only applies to the [Functional API](/oss/javascript/langgraph/functional-api). The [Graph API](/oss/javascript/langgraph/graph-api) re-enters at the node boundary on resume, so node code is not "replayed" from the start of the function the way Temporal-style workflows are.

The Functional API, in contrast, replays the body of an `@entrypoint` from the beginning when a run resumes, using cached [`@task`](https://reference.langchain.com/javascript/langchain-langgraph/index/task) results to skip work that has already been done. Two kinds of changes break this model:
