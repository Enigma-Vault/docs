...
* **Use idempotent operations**: Design side effects so a **node** that retries or re-executes after an interrupt does not corrupt data. Use idempotency keys or read-before-write checks. For how partial **task** failures interact with checkpoints, see [Idempotency](/oss/javascript/langgraph/functional-api#idempotency).
