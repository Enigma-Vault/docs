...
<Note>
    **A `general-purpose` synchronous subagent is enabled by default.** Every deep agent created with `create_deep_agent` ships with a `general-purpose` subagent and the `task` tool wired up — you do not need to pass anything to `subagents=` to get one. Anything you add via `subagents=` is _in addition to_ this default. To replace, rename, or disable it, see [Default subagent](/oss/python/deepagents/subagents#default-subagent).
</Note>
...
**Default `general-purpose` subagent (enabled out of the box):**
- Always present unless you explicitly replace or disable it
- Has the main agent's tools (including filesystem tools) by default
- Can be replaced, renamed, re-prompted, or disabled via the [harness profile](#harness-profiles) — see [Default subagent](/oss/python/deepagents/subagents#default-subagent)

**Custom subagents (added alongside the default):**
- Define specialized subagents with specific tools
- Example: code-reviewer, web-researcher, test-runner
- Configure via the `subagents` parameter — these are added on top of the default `general-purpose` subagent

<Accordion title="Running without subagents (no `task` tool)" icon="ban">
    To run an agent without the `task` tool, see [Running without subagents](/oss/python/deepagents/subagents#running-without-subagents). Don't try removing `SubAgentMiddleware` via `excluded_middleware`—that's intentionally rejected. Instead, disable the auto-added subagent via the [harness profile](#harness-profiles) and pass no synchronous subagents via `subagents=`. Async subagents are unaffected.
</Accordion>
